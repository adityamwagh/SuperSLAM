    ...
    : mT(33.0)  // Initialize mT with a default value
{
  ...
  rec.log("world/current_camera",
          rerun::Image(rerun::Collection<uint8_t>(placeholder_data),
                       placeholder_res, rerun::datatypes::ColorModel::L));
  rec.log("world/current_left_camera",
          rerun::Image(rerun::Collection<uint8_t>(placeholder_data),
                       placeholder_res, rerun::datatypes::ColorModel::L));
  rec.log("world/current_right_camera",
          rerun::Image(rerun::Collection<uint8_t>(placeholder_data),
                       placeholder_res, rerun::datatypes::ColorModel::L));
...
void RerunViewer::SetMap(Map* map) { map_ = map; }

void RerunViewer::SetCameras(float fx_left, float fy_left, float cx_left,
                             float cy_left, float fx_right, float fy_right,
                             float cx_right, float cy_right, float baseline) {
  fx_left_ = fx_left;
  fy_left_ = fy_left;
  cx_left_ = cx_left;
  cy_left_ = cy_left;
...
void RerunViewer::AddCurrentFrame(Frame* current_frame) {
  ...
  // Log current frame image immediately for real-time visualization (even
  ...
  if (current_frame_ && !current_frame_->mImGray.empty()) {
    ...
    if (cameras_set_) {
      rec.log(entity_name,
              rerun::Pinhole::from_focal_length_and_resolution(
                  {fx_left_, fy_left_}, {static_cast<float>(img.cols),
      ...
      rec.log(entity_name, rerun::Pinhole::from_focal_length_and_resolution(
                               {fx, fy}, {static_cast<float>(img.cols),
                                          static_cast<float>(img.rows)}));
    }
    ...
    if (img.channels() == 1) {
      ...
      rec.log(entity_name,
              rerun::Image(rerun::Collection<uint8_t>(img_data), resolution,
                           rerun::datatypes::ColorModel::L));
    } else if (img.channels() == 3) {
      cv::Mat rgb_img;
      cv::cvtColor(img, rgb_img, cv::COLOR_BGR2RGB);
      std::vector<uint8_t> img_data(rgb_img.ptr(),
                                    rgb_img.ptr() + rgb_img.total());
...
void RerunViewer::AddStereoFrames(Frame* current_frame,
  ...
  // Log images immediately for real-time visualization (even during
  // initialization)
  if (current_frame_) {
    rec.set_time_sequence("currentframe_id", current_frame_->mnId);
    ...
    if (!left_image.empty()) {
      ...
      SLOG_DEBUG("Logging left camera image: {}x{}", left_image.cols,
                 left_image.rows);
      ...
      if (cameras_set_) {
        rec.log(left_entity, rerun::Pinhole::from_focal_length_and_resolution(
                                 {fx_left_, fy_left_},
                                 {static_cast<float>(left_image.cols),
                                  static_cast<float>(left_image.rows)}));
      }

      // Log left image
      if (left_image.channels() == 1) {
        std::vector<uint8_t> img_data(left_image.ptr(),
                                      left_image.ptr() + left_image.total());
        ...
        if (!keypoint_positions.empty()) {
          rec.log(left_entity + "/keypoints",
                  rerun::Points2D(keypoint_positions)
                      .with_colors(rerun::Color{0, 255, 0})  // Green keypoints
                      .with_radii(3.0f));
          SLOG_DEBUG("Logged {} keypoints on left camera",
    ...
    if (!right_image.empty()) {
      ...
      SLOG_DEBUG("Logging right camera image: {}x{}", right_image.cols,
                 right_image.rows);
      ...
      if (cameras_set_) {
        rec.log(right_entity, rerun::Pinhole::from_focal_length_and_resolution(
                                  {fx_right_, fy_right_},
                                  {static_cast<float>(right_image.cols),
                                   static_cast<float>(right_image.rows)}));
      }

      // Log right image
      if (right_image.channels() == 1) {
        std::vector<uint8_t> img_data(right_image.ptr(),
                                      right_image.ptr() + right_image.total());
        rerun::WidthHeight resolution(right_image.cols, right_image.rows);
        rec.log(right_entity,
                rerun::Image(rerun::Collection<uint8_t>(img_data), resolution,
                             rerun::datatypes::ColorModel::L));
          ...
          rec.log(right_entity + "/keypoints",
                  rerun::Points2D(keypoint_positions)
                      .with_colors(rerun::Color{
                          255, 0, 0})  // Red keypoints for right camera
                      .with_radii(3.0f));
          SLOG_DEBUG("Logged {} keypoints on right camera",
                     keypoint_positions.size());
        }
      }
    }
...
void RerunViewer::UpdateMap() {
  ...
  // TODO: Implement proper active keyframe logic based on SuperSLAM's
  // requirements
  ...
  if (active_keyframes_.empty()) {
    SLOG_DEBUG(
        "Viewer::UpdateMap - No active keyframes to display (total keyframes: "
        "{}, total landmarks: {})",
        all_keyframes_.size(), active_landmarks_.size());
    return;
  }
  ...
  for (size_t i = 0; i < kf_sort.size(); ++i) {
    ...
    std::string entity_name =
        "world/stereosys" + std::to_string(i) + "/cam_left";

    if (i != 0) {
      // Transform i'th keyframe to coordinate of most recent active frame
      cv::Mat T_wci =
          kf_i->GetPose();  // Camera to world transform for keyframe i
      cv::Mat T_cic0 =
          T_cw0 * T_wci;  // Transform from keyframe i to keyframe 0
      ...
      const Eigen::Vector3f camera_position(
          t_cic0.at<float>(0), t_cic0.at<float>(1), t_cic0.at<float>(2));
    ...
    if (i == 0) {
      ...
      if (!left_img.empty()) {
        if (left_img.channels() == 1) {
          ...
          std::vector<uint8_t> img_data(left_img.ptr(),
                                        left_img.ptr() + left_img.total());
        ...
        if (current_frame_ && !current_frame_->mvKeysUn.empty()) {
          ...
          if (!keypoint_positions.empty()) {
            rec.log(
                entity_name + "/keypoints",
                rerun::Points2D(keypoint_positions)
                    .with_colors(rerun::Color{0, 255, 0})  // Green keypoints
                    .with_radii(3.0f));
            SLOG_INFO("Logged {} keypoints on left camera",
                      keypoint_positions.size());
          }
        }
      }
      ...
      if (has_stereo_images_ && !current_right_image_.empty()) {
        std::string right_entity_name =
            "world/stereosys" + std::to_string(i) + "/cam_right";

        // Right camera pinhole model (same as left but different cx)
        if (cameras_set_) {
          rec.log(right_entity_name,
                  rerun::Pinhole::from_focal_length_and_resolution(
        ...
        if (right_img.channels() == 1) {
          // Convert grayscale to rerun format
          std::vector<uint8_t> img_data(right_img.ptr(),
                                        right_img.ptr() + right_img.total());
            ...
            rec.log(right_entity_name + "/keypoints",
                    rerun::Points2D(keypoint_positions)
                        .with_colors(rerun::Color{
                            255, 0, 0})  // Red keypoints for right camera
                        .with_radii(3.0f));
            SLOG_INFO("Logged {} keypoints on right camera",
                      keypoint_positions.size());
          }
        }
      }
  ...
  // Draw active landmarks (map points) in world coordinates (no transform
  // needed)
  if (!kf_sort.empty()) {
    ...
    for (auto const& [id, mp] : active_landmarks_) {
      if (mp && !mp->isBad()) {
        ...
        if (world_pos.rows == 3 && world_pos.cols == 1) {
          points3d_vector.push_back(rerun::Position3D(world_pos.at<float>(0),
                                                      world_pos.at<float>(1),
                                                      world_pos.at<float>(2)));
        }
      }
    }
    if (!points3d_vector.empty()) {
      rec.log("world/landmarks", rerun::Points3D(points3d_vector)
                                     ...
                                     .with_radii(0.05f));
      SLOG_INFO("Logged {} landmarks", points3d_vector.size());
    } else {
      SLOG_INFO("No valid landmarks to display");
    }
    ...
    // Build trajectory from keyframe poses in world coordinates
    for (const auto& [id, kf] : all_kf_sorted) {
      cv::Mat T_wc = kf->GetPose();
      if (!T_wc.empty() && T_wc.rows == 4 && T_wc.cols == 4 &&
          T_wc.type() == CV_32F) {
        // GetPose() returns T_wc (world from camera), so translation is camera
        // position in world
        cv::Mat t_wc = T_wc.rowRange(0, 3).col(3);
    ...
    if (path.size() > 1) {
      ...
      SLOG_INFO("Trajectory needs at least 2 keyframes (currently: {})",
                path.size());
    }
  }
}
...
void RerunViewer::LogKeyFrame(KeyFrame* keyframe) {
  ...
  std::string msg =
      "New KeyFrame created with ID: " + std::to_string(keyframe->mnId);
  ...
  if (!keyframe->mK.empty()) {
    ...
    std::array<float, 9> intrinsics_flat_array = {fx, 0.f, cx,  0.f, fy,
                                                  cy, 0.f, 0.f, 1.f};
    rec.log(
