...
#define CHECK_RETURN_W_MSG(status, val, errMsg)                          \
  do {                                                                   \
    if (!(status)) {                                                     \
      tensorrt_log::gLogError << errMsg << " Error in " << __FILE__      \
...
#define ASSERT(condition)                                            \
  do {                                                               \
    if (!(condition)) {                                              \
      tensorrt_log::gLogError << "Assertion failure: " << #condition \
                              << std::endl;                          \
      abort();                                                       \
    }                                                                \
  } while (0)
...
std::vector<std::string> classify(const std::vector<std::string>& refVector,
                                  ...
                                  const size_t topK) {
  const auto inds =
      tensorrt_common::argMagnitudeSort(output.cbegin(), output.cend());
...
std::vector<size_t> topKMagnitudes(const std::vector<T>& v, const size_t k) {
  std::vector<size_t> indices =
      tensorrt_common::argMagnitudeSort(v.cbegin(), v.cend());
  indices.resize(k);
  return indices;
}
...
inline void loadLibrary(const std::string& path) {
  ...
  if (handle == nullptr) {
#ifdef _MSC_VER
    tensorrt_log::gLogError << "Could not load plugin library: " << path
                            << std::endl;
#else
    tensorrt_log::gLogError << "Could not load plugin library: " << path
