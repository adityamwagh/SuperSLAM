#!/usr/bin/env python3
"""Evaluate SuperSLAM trajectories against TUM RGB-D ground truth.

  python scripts/benchmarks/evaluate_tum.py --traj-dir results/tum \
      --data-dir ~/datasets/tum --out-dir results/tum

Estimated trajectories are TUM-format files named <sequence>.txt; ground truth is
<data-dir>/<sequence>/groundtruth.txt (TUM format). Associated by timestamp before SE3
alignment. Reports ATE + RPE and an aligned XY plot per sequence.
"""
...
def main() -> None:
    ...
    ap.add_argument(
        "--data-dir", required=True, help="TUM dataset dir (has <seq>/groundtruth.txt)"
    )
