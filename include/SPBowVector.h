 ...
 * bag-of-words representations using DBoW3 vocabulary for loop closure
 * detection. Uses L2-normalized SuperPoint descriptors directly with cosine
 * similarity.
 */
class SPBowVector {
 public:
   ...
   * @param bow_descriptors Output vector of descriptors for DBoW3 (each row as
   * separate Mat)
   */
   ...
   */
  static void filterDescriptorsByResponse(
      const std::vector<cv::KeyPoint>& keypoints, const cv::Mat& descriptors,
      cv::Mat& filtered_descriptors, float min_response = 0.01f);
   ...
   * @param max_features_per_image Maximum features to use per image (default:
   * 500)
   */
