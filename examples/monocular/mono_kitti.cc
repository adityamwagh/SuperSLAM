...
MonocularImages LoadImages(const fs::path &path_to_sequence) {
  ...
  images.image_filenames.resize(nTimes);

  for (int i = 0; i < nTimes; i++) {
    std::stringstream ss;
    ss << std::setfill('0') << std::setw(6) << i;
    images.image_filenames[i] = (prefix_left / (ss.str() + ".png")).string();
  }
...
int main(int argc, char **argv) {
  if (argc != 4) {
    std::cerr << "Usage: ./mono_kitti path_to_vocabulary path_to_settings "
                 "path_to_sequence"
              << std::endl;
    return 1;
  }
