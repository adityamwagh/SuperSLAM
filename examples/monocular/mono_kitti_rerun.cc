...
MonocularImages LoadImages(const fs::path& sequence_path) {
  MonocularImages images;
  
  // For KITTI monocular, we use image_0 directory (left camera)
  const fs::path image_path = sequence_path / "image_0";
  ...
  if (!fs::exists(image_path)) {
    throw std::runtime_error("Image directory does not exist: " + image_path.string());
  }

  SLOG_INFO("Loading from: {}", image_path.string());
  ...
  SLOG_INFO("Found {} images", image_files.size());

  for (size_t i = 0; i < image_files.size(); ++i) {
    images.image_filenames.push_back(image_files[i].string());
  ...
  if (!image_files.empty()) {
    SLOG_DEBUG("First image: {}", images.image_filenames[0]);
  }
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    SLOG_INFO("Loading KITTI monocular images from: {}", path_to_sequence.string());
    MonocularImages images = LoadImages(path_to_sequence);

    int nImages = images.image_filenames.size();
    SLOG_INFO("Loaded {} images", nImages);

    // Create SLAM system with or without visualization
    SLOG_INFO("Initializing SuperSLAM monocular system{}...",
              enable_visualization ? " with Rerun visualization"
                                   : " (no visualization)");

    SuperSLAM::System SLAM(vocab_path.empty() ? "" : vocab_path.c_str(),
                           settings_path.c_str(), SuperSLAM::System::MONOCULAR,
                           enable_visualization);

    SLOG_INFO("\n=== KITTI Monocular SLAM {} ===",
              enable_visualization ? "with Rerun" : "(High Performance Mode)");
    SLOG_INFO("Dataset: {}", path_to_sequence.string());
    SLOG_INFO("Images: {}", nImages);
    ...
    for (int ni = 0; ni < nImages; ni++) {
      ...
      if (im.empty()) {
        SLOG_ERROR("Failed to load image: {}", images.image_filenames[ni]);
        continue;
      }
    ...
    SLOG_INFO("\n=== Performance Summary ===");
    SLOG_INFO("Total frames: {}", nImages);
    ...
    // Save trajectory
    SLOG_INFO("\nSaving trajectories...");
    SLAM.SaveKeyFrameTrajectoryTUM("KeyFrameTrajectory_KITTI_mono.txt");
    SLAM.SaveTrajectoryTUM("CameraTrajectory_KITTI_mono.txt");
    SLAM.SaveTrajectoryKITTI("CameraTrajectory_KITTI_mono_format.txt");

    SLOG_INFO("Saved:");
    SLOG_INFO("- KeyFrameTrajectory_KITTI_mono.txt");
    SLOG_INFO("- CameraTrajectory_KITTI_mono.txt");
    SLOG_INFO("- CameraTrajectory_KITTI_mono_format.txt");
